...
To further interact with the data in the map, the polygon tool (:ref:`interact-with-data-layers-map`) can be used to create summary statistics across spatial areas of interest. Or, the time slider (:ref:`interact-with-data-layers-map`) bar can be used to view the various time intervals of data available.
...
1. From the :ref:`data-catalog`, browse or search for the layers you want to add.
...
Or, follow these steps (:ref:`search-catalog`) for how to add data layers directly from the catalog.


.. _learn-more-about-data-layers-map:
...
.. _interact-with-data-layers-map:
...
The catalog and map offer multiple ways of comparing data within both the mapped interface and within a :ref:`data-catalog`.

For assistance, please contact us via the red Feedback button |feedback_button_icon| in the top right corner of the toolbar.

.. _different-chart-types-map:
...
1. Clicking on any gridded dataset will open a virtual sensor (:ref:`download-time-series-gridded-catalog`).
...
Data may be downloaded through the data catalog, as described in :ref:`download-visualized-data-catalog`.
